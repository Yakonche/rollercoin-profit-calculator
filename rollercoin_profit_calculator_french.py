...
current_hashrate = float(input("Entrez votre hashrate (TH/s) : "))
network_powers = [float(input("Puissance du réseau BTC (EH/s) : ")), float(input("Puissance du réseau DOGE (EH/s) : ")),
                  float(input("Puissance du réseau ETH (EH/s) : "))]
...
print("\n ---------------------------\n\n{} est la crypto-monnaie la plus rentable à exploiter.\nAvec {} € de revenu / "
      "bloc.\nOu {} {} / bloc.\n".format(names[max_index], earnings[max_index], earnings_crypto[max_index],
                                         names[max_index]))
...
print(str(earnings_second*an)+" € / an"+" ,soit "+str(earnings_cryptot*an)+" \n"+names[max_index])
